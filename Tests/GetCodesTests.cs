...
namespace Tests
{
    public class GetCodesTests
    {
        ...
        public void TestRngParams()
        {
            ...
            generator = new TokenGenerator("", saltDefault, 100, 6, TokenGenerator.CodesFileName, CodeType.ALL);
            ...
            generator = new TokenGenerator("test", saltDefault, 100, 6, TokenGenerator.CodesFileName, CodeType.ALL);
            codes = generator.GetCodes(0, 3);
            Assert.That(codes[0] == "K>Aw<V");
            Assert.That(codes[1] == "q0cd;s");
            Assert.That(codes[2] == "jN7+wc");

            generator = new TokenGenerator("", Enumerable.Range(1, 16).Select(x => (byte)x).ToArray(), 1000, 6, TokenGenerator.CodesFileName, CodeType.ALL);
            codes = generator.GetCodes(0, 3);
            Assert.That(codes[0] == "7j\\Wlw");
            Assert.That(codes[1] == "P-oq?*");
            Assert.That(codes[2] == "o9&B+P");
            generator = new TokenGenerator("", Enumerable.Range(1, 16).Select(x => (byte)x).ToArray(), 100, 6, TokenGenerator.CodesFileName, CodeType.ALL);
            codes = generator.GetCodes(0, 3);
        ...
        public void TestCodeTypes()
        {
            ...
            generator = new TokenGenerator("", saltDefault, 1000, 6, TokenGenerator.CodesFileName, CodeType.ALPHABETIC_ONLY_LOWERCASE);
            codes = generator.GetCodes(0, 2);
            Assert.That(codes[0] == "oywcal");
            Assert.That(codes[1] == "ahukdr");

            generator = new TokenGenerator("", saltDefault, 1000, 6, TokenGenerator.CodesFileName, CodeType.ALPHABETIC_ONLY_UPPERCASE);
            codes = generator.GetCodes(0, 2);
            Assert.That(codes[0] == "ORRXDI");
            Assert.That(codes[1] == "CRFBJZ");

            generator = new TokenGenerator("", saltDefault, 1000, 6, TokenGenerator.CodesFileName, CodeType.ALPHABETIC_BOTH);
            codes = generator.GetCodes(0, 2);
            Assert.That(codes[0] == "ORoywR");
            Assert.That(codes[1] == "XDcaIl");

            generator = new TokenGenerator("", saltDefault, 1000, 6, TokenGenerator.CodesFileName, CodeType.NUMERIC);
            codes = generator.GetCodes(0, 2);
            Assert.That(codes[0] == "991583");
            Assert.That(codes[1] == "589604");

            generator = new TokenGenerator("", saltDefault, 1000, 6, TokenGenerator.CodesFileName, CodeType.ALPHANUMERIC);
            codes = generator.GetCodes(0, 2);
        ...
        {
            generator = new TokenGenerator("", saltDefault, 100, 1000, TokenGenerator.CodesFileName, CodeType.ALL);
            Assert.True(Regex.IsMatch(generator.GetCodes(0, 1)[0], "[!-~]+"));

            generator = new TokenGenerator("", saltDefault, 100, 1000, TokenGenerator.CodesFileName, CodeType.ALPHABETIC_ONLY_LOWERCASE);
            Assert.True(Regex.IsMatch(generator.GetCodes(0, 1)[0], "[a-z]+"));

            generator = new TokenGenerator("", saltDefault, 100, 1000, TokenGenerator.CodesFileName, CodeType.ALPHABETIC_ONLY_UPPERCASE);
            Assert.True(Regex.IsMatch(generator.GetCodes(0, 1)[0], "[A-Z]+"));

            generator = new TokenGenerator("", saltDefault, 100, 1000, TokenGenerator.CodesFileName, CodeType.ALPHABETIC_BOTH);
            Assert.True(Regex.IsMatch(generator.GetCodes(0, 1)[0], "[a-zA-Z]+"));

            generator = new TokenGenerator("", saltDefault, 100, 1000, TokenGenerator.CodesFileName, CodeType.NUMERIC);
            Assert.True(Regex.IsMatch(generator.GetCodes(0, 1)[0], "[0-9]+"));

            generator = new TokenGenerator("", saltDefault, 100, 1000, TokenGenerator.CodesFileName, CodeType.ALPHANUMERIC);
